...
USE MISC_FUNCTIONS, ONLY: EVALUATE_RAMP
...
IF (ALLOCATED(ML%I_RAMP_A)) THEN
   IF (ML%I_RAMP_A(NR) > 0) A_LOC = EVALUATE_RAMP(TMP,ML%I_RAMP_A(NR))
ENDIF

E_LOC = ML%E(NR)
IF (ALLOCATED(ML%I_RAMP_E)) THEN
   IF (ML%I_RAMP_E(NR) > 0) E_LOC = 1000._EB*EVALUATE_RAMP(TMP,ML%I_RAMP_E(NR))
ENDIF

IF (ABS(E_LOC) < TWENTY_EPSILON_EB) THEN
   RR_MAX = A_LOC*RHO_S**ML%N_S(NR)
ELSE
   RR_MAX = 0._EB
ENDIF
...
DO WHILE (INT(TMP)<I_MAX_TEMP)
   ...
   REACTION_RATE = A_LOC*RHO_S**ML%N_S(NR)*EXP(-E_LOC/(R0*TMP))*TMP**ML%N_T(NR)*ML%X_O2_PYRO**ML%N_O2(NR)
