...
REAL(EB) :: T_PRINT_C
INTEGER, SAVE :: LAST_PRINT_BIN = -999999
B_NUMBER = 0._EB
Q_DOT_S_PPP = 0._EB
Q_DOT_G_PPP = 0._EB
Q_DOT_O2_PPP = 0._EB
...
ALPHA_LOC = MAX(0._EB, MIN(1._EB, ALPHA_LOC))

F_ALPHA_LOC = (1._EB - ALPHA_LOC)**ML%N_S(J)

IF (ALLOCATED(ML%I_RAMP_F_ALPHA)) THEN
   IF (ML%I_RAMP_F_ALPHA(J) > 0) F_ALPHA_LOC = EVALUATE_RAMP(ALPHA_LOC,ML%I_RAMP_F_ALPHA(J))
ENDIF
            
  IF (ALLOCATED(ML%I_RAMP_F_ALPHA)) THEN
   IF (ML%I_RAMP_F_ALPHA(J) > 0) THEN
      REACTION_RATE = RHO0_LOC*A_LOC*EXP(-E_LOC/(R0*TMP_S))*F_ALPHA_LOC
   ELSE
