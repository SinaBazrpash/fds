...
CHARACTER(LABEL_LENGTH), DIMENSION(MAX_REACTIONS) :: RAMP_A, RAMP_E
CHARACTER(LABEL_LENGTH), DIMENSION(MAX_REACTIONS) :: RAMP_F_ALPHA
...
NAMELIST /MATL/ A,ABSORPTION_COEFFICIENT,ADJUST_H,ALLOW_SHRINKING,ALLOW_SWELLING,BOILING_TEMPERATURE,&
                CONDUCTIVITY,CONDUCTIVITY_RAMP,DENSITY,E,RAMP_A, RAMP_E,RAMP_F_ALPHA,EMISSIVITY,FYI,&
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   ML%FYI                              = FYI
   ALLOCATE(ML%RAMP_A(N_REACTIONS),STAT=IZERO)
   CALL ChkMemErr('READ','ML%RAMP_A',IZERO)
   ML%RAMP_A(1:N_REACTIONS) = RAMP_A(1:N_REACTIONS)
   ALLOCATE(ML%RAMP_E(N_REACTIONS),STAT=IZERO)
   CALL ChkMemErr('READ','ML%RAMP_E',IZERO)
   ML%RAMP_E(1:N_REACTIONS) = RAMP_E(1:N_REACTIONS)
   ALLOCATE(ML%I_RAMP_A(N_REACTIONS),STAT=IZERO)
   CALL ChkMemErr('READ','ML%I_RAMP_A',IZERO)
   ML%I_RAMP_A = 0
   ALLOCATE(ML%I_RAMP_E(N_REACTIONS),STAT=IZERO)
   CALL ChkMemErr('READ','ML%I_RAMP_E',IZERO)
   ML%I_RAMP_E = 0
   ALLOCATE(ML%RAMP_F_ALPHA(N_REACTIONS),STAT=IZERO)
   CALL ChkMemErr('READ','ML%RAMP_F_ALPHA',IZERO)
   ML%RAMP_F_ALPHA(1:N_REACTIONS) = RAMP_F_ALPHA(1:N_REACTIONS)
   ALLOCATE(ML%I_RAMP_F_ALPHA(N_REACTIONS),STAT=IZERO)
   CALL ChkMemErr('READ','ML%I_RAMP_F_ALPHA',IZERO)
   ML%I_RAMP_F_ALPHA = 0
...
DO NR=1,ML%N_REACTIONS
   IF (ML%RAMP_A(NR)/='null') CALL GET_RAMP_INDEX(ML%RAMP_A(NR),'TEMPERATURE',ML%I_RAMP_A(NR))
   IF (ML%RAMP_E(NR)/='null') CALL GET_RAMP_INDEX(ML%RAMP_E(NR),'TEMPERATURE',ML%I_RAMP_E(NR))
ENDDO

DO NR=1,ML%N_REACTIONS
   IF ((ML%RAMP_A(NR)/='null' .OR. ML%RAMP_E(NR)/='null') .AND. ML%TMP_REF(NR) > 0._EB) THEN
      WRITE(MESSAGE,'(5A,I0)') 'ERROR: MATL ',TRIM(ML%ID), &
           ' cannot use RAMP_A/RAMP_E together with REFERENCE_TEMPERATURE for reaction ', NR
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
ENDDO

DO NR=1,ML%N_REACTIONS
   IF (ML%RAMP_F_ALPHA(NR)/='null') CALL GET_RAMP_INDEX(ML%RAMP_F_ALPHA(NR),'FRACTION',ML%I_RAMP_F_ALPHA(NR))
ENDDO
